...
| E2E-003 | CRITICAL | Fixture Agent quality is presented as real-model quality | Dataset/manifest, fixed baselines, six-case real-model calibration and ADR 0013 thresholds exist at exact SHA `4c20344`; quality split and real-model hostile-input surveys are not yet run | Run the frozen thirteen-case quality split, deterministic gate, prompt-injection survey and secret-canary survey with 3 runs/case and zero safety violations | NOT RUN; AGENT_QUALITY BLOCKER |
