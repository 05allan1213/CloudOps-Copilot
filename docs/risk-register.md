...
| E2E-003 | CRITICAL | Fixture Agent quality is presented as real-model quality | Exact-SHA `f8ffcc1` quality run completed all 39 runs with zero safety violations, but insufficient recall was 0.80 and `model-conflicting-sources` failed 3/3 because a ready identity policy conflicts with the insufficient oracle; hostile-input surveys were not run | Authorize a new frozen eval revision or deterministic policy repair, then repeat baseline, calibration, threshold verification, quality and hostile-input surveys from new exact hashes | FAIL; AGENT_QUALITY BLOCKER |
