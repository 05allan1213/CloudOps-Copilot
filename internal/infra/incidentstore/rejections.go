...
func validateRejection(rejection RejectionInput) error {
	...
	if len(rejection.Source) > 64 || len(rejection.SourceEventID) != 64 || len(rejection.Fingerprint) > 128 ||
		len(rejection.AlertInstanceKey) != 64 || (rejection.CorrelationKey != "" && len(rejection.CorrelationKey) != 64) || len(rejection.ReasonCode) > 64 {
		return errors.New("rejection identity exceeds schema bounds")
	}
	if rejection.Source != "alertmanager" {
		return errors.New("rejection source identity is not canonical Alertmanager")
	}
	for name, digest := range map[string]string{
		"source event id":    rejection.SourceEventID,
