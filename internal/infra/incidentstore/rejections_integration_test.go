...
func TestMySQLIncidentTargetRejectionIsDurableAndIdempotent(t *testing.T) {
	...
	rejection := RejectionInput{
		Source: "alertmanager", SourceEventID: strings.Repeat("a", 64),
	...
	assertIncidentIntegrationCount(t, ctx, db, "SELECT COUNT(*) FROM incidents", 0)
