...
func decideNoChangeWorkflow(state noChangeWorkflowState) noChangeDecision {
	if state.ActiveVerification || state.IncidentStatus == domain.StatusVerifying {
		return noChangeDecision{Reason: "verification_already_active"}
	}
	if state.IncidentStatus == domain.StatusResolved || state.IncidentStatus == domain.StatusClosed {
	...
	if state.Change != nil {
		if state.IncidentStatus != domain.StatusDelivering {
	...
	case domain.StatusDetected, domain.StatusInvestigating:
...
func startNoChangeVerification(ctx context.Context, tx *sql.Tx, incident *incidentRow, triggerSignalID uint64) (bool, error) {
	...
	if !budget.Allowed() {
		...
		if err := businessbudget.MarkExhausted(ctx, tx, budget, incident.id, uint32(incident.cycleNo), "incident-ingress.no-change"); err != nil {
			return false, err
		}
		incident.version++
		return false, nil
...
ORDER BY id LIMIT 2 FOR UPDATE`, incident.id, incident.cycleNo)
	...
	plan := &noChangePlanState{}
	err = tx.QueryRowContext(ctx, `SELECT id, status, row_version
...
ORDER BY id LIMIT 1 FOR UPDATE`, incident.id, incident.cycleNo).Scan(&plan.ID, &plan.Status, &plan.RowVersion)
	...
	err = tx.QueryRowContext(ctx, `SELECT cr.id, cr.plan_id, cr.status, rp.status,
...
WHERE cr.incident_id = ? AND cr.cycle_no = ?
  AND cr.status IN ('pending','pr_open','merged','syncing','rolling_out')
...
JOIN baseline_observations o ON o.baseline_id = b.id
  AND o.observation_type = 'argocd_revision'
...
WHERE incident_id = ? AND cycle_no = ?
  AND status IN ('awaiting_approval','approved')`, incident.id, incident.cycleNo)
		if err != nil {
			return noChangeCancellation{}, err
		}
...
SET status = 'cancelled', ci_status = 'cancelled',
...
WHERE incident_id = ? AND cycle_no = ? AND status = 'pending'
...
func ensureNoChangeInvestigating(ctx context.Context, tx *sql.Tx, incident *incidentRow, triggerSignalPublicID string) error {
	if incident.status == domain.StatusInvestigating {
		return nil
	}
...
WHERE id = ? AND cycle_no = ? AND version = ? AND status = ?`,
	...
	incident.status = domain.StatusInvestigating
	incident.version++
	metadata, _ := json.Marshal(map[string]any{"from": previous, "trigger_signal_id": triggerSignalPublicID})
	return appendEvent(ctx, tx, *incident, "incident_returned_to_investigating", "system", "incident-ingress",
...
WHERE id = ? AND cycle_no = ? AND version = ? AND status = 'investigating'`,
	...
	return appendEvent(ctx, tx, *incident, "no_change_verification_blocked", "system", "incident-ingress",
...
func insertNoChangeVerification(ctx context.Context, tx *sql.Tx, incident *incidentRow, triggerSignalID uint64, attempt int, snapshot noChangeVerificationSnapshot, budget businessbudget.Result) (bool, error) {
 ...
 (public_id, incident_id, cycle_no, originating_agent_run_id,
  business_budget_authorization_id, remediation_plan_id, change_request_id,
  status, trigger_type, trigger_signal_id, target_revision, source_revision, image_digest,
...
VALUES (?, ?, ?, ?, ?, NULL, NULL, 'pending', 'no_change_signal', ?, ?, ?, ?, ?, ?, ?, ?,
        1, ?, 60000, ?, ?, 1, 1, ?, ?, ?)`,
 ...
 (public_id, verification_run_id, incident_id, cycle_no, check_type,
...
WHERE id = ? AND cycle_no = ? AND version = ? AND status = 'investigating'`,
	...
	incident.status = domain.StatusVerifying
	incident.version++
	metadata, _ := json.Marshal(map[string]any{
	...
	if err := appendEvent(ctx, tx, *incident, "verification_started", "system", "incident-ingress",
...
VALUES (?, 1, ?, ?, ?, ?, 'change_request_cancelled_before_write', 'system', NULL,
...
func appendNoChangeCancellationEvent(ctx context.Context, tx *sql.Tx, incident incidentRow, triggerSignalPublicID string, decision noChangeDecision, cancelled noChangeCancellation) error {
	...
	return appendEvent(ctx, tx, incident, "no_change_work_cancelled", "system", "incident-ingress",
...
func appendNoChangeDeferredEvent(ctx context.Context, tx *sql.Tx, incident incidentRow, triggerSignalPublicID, reason string) error {
	...
	return appendEvent(ctx, tx, incident, "no_change_verification_deferred", "system", "incident-ingress",
