// Package incidentstore owns Incident ingress transactions.
...
func NewStore(db *sql.DB) (*Store, error) {
	if db == nil {
		return nil, errors.New("incident database is required")
	}
...
func (s *Store) Ready(ctx context.Context) error {
	...
	for _, table := range []string{"async_tasks", "async_task_attempts", "signal_rejections", "command_idempotency_records", "data_import_audits"} {
		var count int
...
WHERE id = ? AND cycle_no = ? AND version = ?`, incoming, incident.id, incident.cycleNo, incident.version)
			if err != nil {
				return nil, err
			}
	...
	if createdOrReopened && firing > 0 {
		...
		if created {
			if err := appendLifecycleEvent(ctx, tx, incident, "investigation_start_enqueued", "system", "incident-ingress", "investigation start task enqueued"); err != nil {
				return nil, err
			}
...
INSERT INTO incident_correlation_locks (correlation_key, correlation_key_version, touched_at)
VALUES (?, 2, NOW(6))
ON DUPLICATE KEY UPDATE touched_at = NOW(6)`, key); err != nil {
		return err
	}
	var keyVersion uint64
	if err := tx.QueryRowContext(ctx, `SELECT correlation_key_version
FROM incident_correlation_locks WHERE correlation_key = ? FOR UPDATE`, key).Scan(&keyVersion); err != nil {
		return err
	}
	if keyVersion != correlationKeyVersion {
		return errors.New("correlation key has an unsupported identity version")
	}
	return nil
}
...
func selectActiveIncident(ctx context.Context, tx *sql.Tx, key string) (incidentRow, error) {
	var row incidentRow
	err := tx.QueryRowContext(ctx, `
SELECT id, public_id, cycle_no, severity, status, version,
       cluster, environment, namespace, service_name, target_kind, target_name, migrated_legacy_context
FROM incidents
WHERE active_correlation_key = CONVERT(? USING binary)
...
func createOrReopenIncident(ctx context.Context, tx *sql.Tx, input SignalInput) (incidentRow, bool, error) {
	...
	err := tx.QueryRowContext(ctx, `
SELECT id, public_id, cycle_no, severity, status, version, migrated_legacy_context,
       (status = 'resolved' AND terminal_at >= TIMESTAMPADD(MINUTE, -30, NOW(6)))
FROM incidents
WHERE correlation_key = ? AND status IN ('resolved','closed')
ORDER BY terminal_at DESC, id DESC LIMIT 1 FOR UPDATE`, input.CorrelationKey).
	...
	if err == nil && latest.status == domain.StatusResolved && withinWindow {
	...
	UPDATE incidents
	SET cycle_no = cycle_no + 1, version = version + 1, status = 'investigating',
  ...
  AND terminal_at >= TIMESTAMPADD(MINUTE, -30, NOW(6))`, severity, input.OccurredAt.UTC(), latest.id, latest.version)
		...
		latest.cycleNo++
		latest.version++
		latest.status = domain.StatusInvestigating
		latest.severity = severity
		if err := appendLifecycleEvent(ctx, tx, latest, "incident_reopened", "system", "incident-ingress", "incident reopened in a new cycle"); err != nil {
...
INSERT INTO incidents
    (public_id, fingerprint, correlation_key, correlation_key_version, cluster, namespace,
     ...
     first_seen_at, last_seen_at, resolved_at, version, cycle_no, needs_attention, created_at, updated_at)
VALUES (?, ?, ?, 2, ?, ?, ?, ?, ?, ?, ?, 'detected', ?, ?, ?, NULL, 1,
	...
	row := incidentRow{
		id: uint64(id), publicID: publicID, cycleNo: 1, severity: input.Severity, status: domain.StatusDetected, version: 1,
	...
	if err := appendLifecycleEvent(ctx, tx, row, "incident_created", "system", "incident-ingress", "incident created"); err != nil {
		return incidentRow{}, false, err
	}
	return row, true, nil
}
...
SET incident_id = ?, cycle_no = ?, canonical_schema_version = 2,
    correlation_key_version = 2
WHERE id = ? AND incident_id IS NULL AND cycle_no IS NULL
  AND canonical_schema_version IS NULL AND correlation_key_version IS NULL`, incident.id, incident.cycleNo, signalID)
	if err != nil {
		return err
	}
	if affected, _ := result.RowsAffected(); affected != 1 {
...
func enqueueInvestigationStartWithAuthorization(ctx context.Context, tx *sql.Tx, incident incidentRow, authorizationPublicID string) (bool, bool, error) {
	...
	if !budget.Allowed() {
		if err := businessbudget.MarkExhausted(ctx, tx, budget, incident.id, uint32(incident.cycleNo), "incident-ingress"); err != nil {
			return false, false, err
		}
...
WHERE id = ? AND status = 'ready' AND lease_generation = ?
  AND expected_subject_version = ?`, incident.version, dedupe, taskID, leaseGeneration, expectedVersion)
		...
		if err := appendEvent(ctx, tx, incident, "investigation_start_refreshed", "system", "incident-ingress",
...
WHERE id = ? AND status = 'running' AND lease_owner = ? AND lease_generation = ?
  AND expected_subject_version = ?`, taskID, leaseOwner.String, leaseGeneration, expectedVersion)
	...
	if err := appendEvent(ctx, tx, incident, "investigation_start_replaced", "system", "incident-ingress",
...
SELECT s.incident_id, s.cycle_no, i.public_id, i.severity, i.status, i.version
...
WHERE s.source = ? AND s.fingerprint = ? AND s.starts_at = ? AND s.status = 'firing'
  AND s.incident_id IS NOT NULL AND s.cycle_no IS NOT NULL
  AND i.correlation_key = ?
ORDER BY s.id
LIMIT 2
