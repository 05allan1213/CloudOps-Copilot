...
func TestMySQLIncidentDuplicateCreateReopenAndStartUniqueness(t *testing.T) {
	...
	correlationKey := strings.Repeat("a", 64)
...
WHERE incident_id = ? AND cycle_no = 1`, incidentID).Scan(&cycleOneRunID); err != nil {
		t.Fatal(err)
	}

	if _, err := db.ExecContext(ctx, `UPDATE agent_runs
SET status = 'completed', completed_at = NOW(6), row_version = row_version + 1
WHERE incident_id = ? AND cycle_no = 1`, incidentID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.ExecContext(ctx, `UPDATE incidents
SET status = 'resolved', resolved_at = NOW(6), terminal_at = NOW(6),
    current_agent_run_id = ?, version = version + 1
WHERE id = ? AND cycle_no = 1`, cycleOneRunID, incidentID); err != nil {
		t.Fatal(err)
	}
	...
	for cycle := 1; cycle <= 2; cycle++ {
		assertIncidentIntegrationCount(t, ctx, db, "SELECT COUNT(*) FROM agent_runs WHERE incident_id = ? AND cycle_no = ?", 1, incidentID, cycle)
...
func TestMySQLIncidentAgentRunBudgetStopsConcurrentStarts(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	store, err := NewStore(db)
...
FROM agent_runs WHERE incident_id = ? AND cycle_no = 1`, taskhandler.DefaultAgentRunBudget, incidentID)
	assertIncidentIntegrationCount(t, ctx, db, `SELECT COUNT(*)
...
func TestMySQLIncidentAuthorizedAgentRunSlotsFourAndFiveAreConcurrentAndIdempotent(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := strings.Repeat("b", 64)
	...
	for slot := businessbudget.DefaultLimit + 1; slot <= businessbudget.HardLimit; slot++ {
		authorization, version := authorizeBudgetAgentRun(t, ctx, db, incidentID, fmt.Sprintf("Owner authorized slot %d after reviewing durable evidence", slot))
...
WHERE incident_id = ? AND cycle_no = 1`, slot, incidentID)
		assertIncidentIntegrationCount(t, ctx, db, `SELECT COUNT(*) FROM agent_runs
...
WHERE incident_id = ? AND event_type = 'agent_run_created'
  AND JSON_UNQUOTE(JSON_EXTRACT(metadata_json, '$.business_budget_authorization_id')) = ?`, 1, incidentID, authorization.PublicID)
	...
	_, hard, err := businessbudget.AuthorizeAgentRun(ctx, tx, incidentID, 1, businessbudget.Actor{
		Provider: "local", Login: "owner", Role: "owner", Reason: "slot six must reject", RequestID: uuid.NewString(),
	})
	if err != nil || hard.Outcome != businessbudget.OutcomeHardExhausted {
		_ = tx.Rollback()
		t.Fatalf("hard authorization result=%+v err=%v", hard, err)
	}
	if err := businessbudget.MarkExhausted(ctx, tx, hard, incidentID, 1, "integration.owner"); err != nil {
		_ = tx.Rollback()
		t.Fatal(err)
	}
...
func TestMySQLIncidentAutomaticStartProducersCreateNoOrphanAtDefaultBudget(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	created, err := store.IngestBatch(ctx, []SignalInput{incidentIntegrationSignal(46, strings.Repeat("d", 64))})
	if err != nil || len(created) != 1 {
...
func TestMySQLIncidentBudgetAuthorizationRejectsMissingWrongCycleAndWrongLineage(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	...
	_, _, err = businessbudget.AuthorizeAgentRun(ctx, tx, incidentID, 1, businessbudget.Actor{
		Provider: "local", Login: "owner", Role: "owner", RequestID: uuid.NewString(),
	})
	_ = tx.Rollback()
	...
	authorization, _ := authorizeBudgetAgentRun(t, ctx, db, incidentID, "Owner approved slot four with durable evidence")
...
func TestMySQLIncidentPlanAndBothVerificationPathsShareDurableAuthorizedLineage(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	incidentID, automaticRunID := insertSimpleBudgetIncident(t, ctx, db, businessbudget.DefaultLimit)
	authorization, _ := authorizeBudgetAgentRun(t, ctx, db, incidentID, "Owner authorized a bounded child-producing investigation retry")
	...
	if _, err := db.ExecContext(ctx, `INSERT INTO agent_runs
	 (public_id, incident_id, model, prompt_version, max_steps, failure_code,
	  completed_at, row_version, status, cycle_no,
	  expected_incident_version, business_budget_authorization_id)
	VALUES (?, ?, 'fixture-model', 'incident-investigation-fixture', 1, '', NOW(6), 1,
	...
	for _, test := range []struct {
		...
		{name: "no change verification", source: "incident-ingress.no-change", kind: businessbudget.KindVerificationRun},
	} {
...
func TestMySQLIncidentConcurrentCreate(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
...
func TestMySQLIncidentMultiAlertSingleDecisionUsesStrongestSeverity(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
...
func TestMySQLIncidentSequentialSeverityRefreshesReadyStartTask(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := strings.Repeat("7", 64)
...
func TestMySQLIncidentSequentialSeverityFencesRunningStartTask(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := strings.Repeat("8", 64)
...
func TestMySQLIncidentSequentialSeverityPreservesRunningAuthorizedStart(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := strings.Repeat("e", 64)
	...
	authorization, authorizedVersion := authorizeBudgetAgentRun(t, ctx, db, incidentID, "Owner approved retry despite a concurrent severity refresh")
...
func TestMySQLIncidentResolvedWithoutActiveIncidentAttachesToOriginalFiring(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := strings.Repeat("9", 64)
...
WHERE incident_id = ? AND transition = 'investigation.start' AND status = 'ready'`, incidentID); err != nil {
	...
	resolved := firing
	resolved.SourceEventID = strings.Repeat("a", 64)
...
func TestMySQLIncidentSameBatchResolvedBeforeFiringStillPairsInstance(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	firing.SourceEventID = strings.Repeat("f", 64)
	resolved := firing
	resolved.SourceEventID = strings.Repeat("0", 64)
	resolved.Status = domain.SignalStatusResolved
	endsAt := firing.StartsAt.Add(time.Minute)
	resolved.EndsAt = &endsAt
	resolved.OccurredAt = endsAt
...
func TestMySQLIncidentReopenWindowAndLatestTerminalRules(t *testing.T) {
	t.Run("exact thirty minute boundary reopens", func(t *testing.T) {
		...
		correlationKey := strings.Repeat("d", 64)
		...
		if !reopened || row.cycleNo != 2 || row.status != domain.StatusInvestigating || row.severity != domain.SeverityWarning {
	...
	t.Run("newer closed terminal prevents older resolved reopen", func(t *testing.T) {
		...
		correlationKey := strings.Repeat("f", 64)
		...
		assertIncidentIntegrationCount(t, ctx, db, "SELECT COUNT(*) FROM incidents WHERE id = ? AND status = 'resolved' AND cycle_no = 1", 1, resolvedID)
		assertIncidentIntegrationCount(t, ctx, db, "SELECT COUNT(*) FROM incidents WHERE id = ? AND status = 'closed' AND cycle_no = 1", 1, closedID)
	})
}
...
FROM agent_runs r
	WHERE r.incident_id = ? AND r.cycle_no = ?
  AND r.active_incident_cycle_key IS NOT NULL AND r.status IN ('pending','running')
...
JOIN agent_runs r
  ON r.incident_id = i.id AND r.cycle_no = i.cycle_no
	 AND r.active_incident_cycle_key IS NOT NULL
 AND r.status IN ('pending','running')
	WHERE i.id = ? AND i.status = 'investigating'
...
WHERE id = ? AND status = 'pending'`, runID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.ExecContext(ctx, `
UPDATE incidents
SET version = version + 1, updated_at = NOW(6)
WHERE id = ? AND status = 'investigating' AND version = ?`, incidentID, version); err != nil {
		t.Fatal(err)
	}
}
...
func insertSimpleBudgetIncident(t *testing.T, ctx context.Context, db *sql.DB, agentRuns int) (uint64, uint64) {
	...
	VALUES (?, ?, ?, 2, 'kind', 'demo', 'checkout', 'demo', 'Deployment', 'checkout',
	        'warning', 'simple business budget fixture', NOW(6), NOW(6), 1, 'investigating', 1)`,
		publicID, "simple-budget-"+publicID, hashCanonical("simple-budget", publicID))
	if err != nil {
		t.Fatal(err)
	}
...
func insertPartialBudgetPlans(t *testing.T, ctx context.Context, db *sql.DB, incidentID uint64, count int) {
	...
	for index := 1; index <= count; index++ {
	 ...
	 (public_id, incident_id, plan_version, plan_hash, operation_type,
	  ...
	  cycle_no, status, hash_schema_version, canonical_plan_hash)
	VALUES (?, ?, ?, ?, 'restore_required_env', 'acme/gitops', ?, 'apps/demo.yaml',
	        JSON_OBJECT(), JSON_ARRAY(), 'low', ?, ?, ?, 'budget fixture', 'manual rollback',
	        'manual validation', 1, 1, 'cancelled', 1, ?)`,
...
func insertPartialBudgetVerifications(t *testing.T, ctx context.Context, db *sql.DB, incidentID uint64, count int) {
	...
	VALUES (?, ?, 'alertmanager', ?, ?, 'resolved', 'warning', 'kind', 'demo', 'checkout',
	        ...
	        JSON_OBJECT(), JSON_OBJECT(), 1, 2, 2, ?, NOW(6), NOW(6))`,
			signalPublicID, incidentID, fmt.Sprintf("%064x", sequence), fmt.Sprintf("budget-fingerprint-%d", sequence), fmt.Sprintf("%064x", sequence))
		if err != nil {
			t.Fatal(err)
		}
		signalID, _ := result.LastInsertId()
		if _, err := db.ExecContext(ctx, `INSERT INTO verification_runs
	 (public_id, incident_id, remediation_plan_id, change_request_id, target_revision,
	  plan_json, deadline_at, attempt, row_version, cycle_no, status,
	...
	VALUES (?, ?, NULL, NULL, ?, JSON_OBJECT(), NOW(6), 1, 1, 1, 'cancelled',
...
func authorizeBudgetAgentRun(t *testing.T, ctx context.Context, db *sql.DB, incidentID uint64, reason string) (businessbudget.Authorization, uint64) {
	...
	authorization, result, err := businessbudget.AuthorizeAgentRun(ctx, tx, incidentID, 1, businessbudget.Actor{
		Provider: "local", Login: "owner", Role: "owner", Reason: reason, RequestID: uuid.NewString(),
	})
...
func incidentIntegrationSignal(sequence int, correlationKey string) SignalInput {
	...
	return SignalInput{
		Source: "alertmanager", SourceEventID: fmt.Sprintf("%064x", sequence),
...
summary, first_seen_at, last_seen_at, resolved_at, version,
status, cycle_no, terminal_at
) VALUES (?, 'terminal-fixture', ?, 2, 'kind', 'demo', 'checkout', 'demo',
	          'Deployment', 'checkout', 'warning', 'terminal fixture',
	          TIMESTAMPADD(HOUR, -1, NOW(6)), NOW(6), %s, 1, ?, 1, %s)`,
		resolvedExpression, terminalExpression)
	result, err := executor.ExecContext(ctx, query, uuid.NewString(), correlationKey, status)
	if err != nil {
		return 0, err
	}
...
func openIncidentIntegrationDB(t *testing.T) *sql.DB {
	...
	name := fmt.Sprintf("cloudops_incident_%d", time.Now().UnixNano())
