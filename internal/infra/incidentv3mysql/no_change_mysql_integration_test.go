...
func TestMySQLIncidentV3ResolvedSignalCreatesNoChangeVerificationAndFencesAgent(t *testing.T) {
	...
	correlationKey := "v2:" + strings.Repeat("4", 64)
...
VALUES (?, ?, 'COMPLETED', 'legacy-model', 'legacy-v2', 1, '', NOW(6))`, uuid.NewString(), incidentID)
	...
	var incidentStatus, legacyStatus string
	var incidentVersion uint64
	var currentAgent sql.NullInt64
	if err := db.QueryRowContext(ctx, `SELECT v3_status, status, version, current_agent_run_id
...
WHERE incident_id = ? AND cycle_no = 1 AND v3_status = 'cancelled' AND status = 'CANCELLED'`, 1, incidentID)
...
FROM verification_runs WHERE incident_id = ? AND cycle_no = 1`, incidentID).Scan(
	...
	if json.Unmarshal(planJSON, &plan) != nil || verification.ValidateV3Plan(plan) != nil ||
...
func TestMySQLIncidentV3NoChangeRequiresFinalEmptyFiringSet(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	...
	correlationKey := "v2:" + strings.Repeat("5", 64)
...
func TestMySQLIncidentV3MissingBaselineCancelsAgentAndBlocksNoChange(t *testing.T) {
	db := openIncidentIntegrationDB(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	store, err := NewStore(db)
	...
	correlationKey := "v2:" + strings.Repeat("6", 64)
...
WHERE incident_id = ? AND cycle_no = 1`, 0, incidentID)
	assertIncidentIntegrationCount(t, ctx, db, `SELECT COUNT(*) FROM agent_runs
WHERE incident_id = ? AND cycle_no = 1 AND v3_status = 'cancelled'`, 1, incidentID)
	var status, reason string
	var attention bool
	if err := db.QueryRowContext(ctx, `SELECT v3_status, needs_attention, blocking_reason_code
...
VALUES (?, 3, 1, ?, 'kind', 'demo', 'demo', 'Deployment', 'checkout', 'checkout',
...
VALUES (?, 3, 1, ?, 1, 'argocd_revision', 'argocd/application', ?, ?, ?, NOW(6), NOW(6))`,
...
func resolvedIncidentIntegrationSignal(firing SignalInput, sequence int) SignalInput {
	resolved := firing
	resolved.SourceEventID = fmt.Sprintf("v2:%064x", sequence)
