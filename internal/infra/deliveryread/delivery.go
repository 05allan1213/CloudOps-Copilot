...
// DeliveryGitHubReader owns the extra exact-tree, post-image and CI producer checks
// required by delivery.observe. The legacy GitHubReader intentionally does not
// claim those stronger facts.
type DeliveryGitHubReader interface {
...
type DeliveryGitHubConfig struct {
...
// DeliveryGitHub upgrades the existing bounded GitHub read client with the exact
// tree/content and required-check identity contract expected by
// delivery.observe. It performs reads only.
type DeliveryGitHub struct{ config DeliveryGitHubConfig }

func NewDeliveryGitHub(config DeliveryGitHubConfig) (*DeliveryGitHub, error) {
	...
	if config.Reader == nil || config.RequiredCheckName == "" || config.ProducerAppID <= 0 || config.WorkflowID <= 0 ||
		...
		return nil, errors.New("GitHub delivery reader requires exact check App and workflow identity")
	}
	return &DeliveryGitHub{config: config}, nil
}

func (g *DeliveryGitHub) ObservePullRequest(ctx context.Context, request taskhandler.DeliveryObserveRequest) (taskhandler.DeliveryPullRequestObservation, error) {
...
func (g *DeliveryGitHub) ObserveCI(ctx context.Context, request taskhandler.DeliveryObserveRequest) (taskhandler.DeliveryCIObservation, error) {
	repository, err := repositoryRef(request.Repository)
...
func (g *DeliveryGitHub) commitIdentity(ctx context.Context, repository change.RepositoryRef, revision, targetPath string) (change.Commit, string, error) {
...
// DeliveryObserver composes the existing bounded Argo, Kubernetes and Registry read
// ports while preserving the one requested delivery step. The rollout step
...
func NewDeliveryObserver(
	github DeliveryGitHubReader,
	argo change.ArgoCDReader,
	rollout verification.RolloutReader,
	runtime change.RuntimeReader,
	registry change.RegistryMetadataReader,
) (*DeliveryObserver, error) {
	if github == nil || argo == nil || rollout == nil || runtime == nil || registry == nil {
		return nil, errors.New("delivery observer requires GitHub, Argo, Kubernetes rollout/runtime, and Registry readers")
	}
	return &DeliveryObserver{github: github, argo: argo, rollout: rollout, runtime: runtime, registry: registry}, nil
}

func (o *DeliveryObserver) Observe(ctx context.Context, request taskhandler.DeliveryObserveRequest) (taskhandler.DeliveryObservation, error) {
	if o == nil {
...
func (o *DeliveryObserver) observeArgo(ctx context.Context, request taskhandler.DeliveryObserveRequest) (taskhandler.DeliveryArgoObservation, error) {
...
func (o *DeliveryObserver) observeRollout(ctx context.Context, request taskhandler.DeliveryObserveRequest) (taskhandler.DeliveryRolloutObservation, error) {
...
var _ taskhandler.DeliveryObserver = (*DeliveryObserver)(nil)
var _ DeliveryGitHubReader = (*DeliveryGitHub)(nil)
