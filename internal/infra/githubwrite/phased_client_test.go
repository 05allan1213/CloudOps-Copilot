...
func TestPhasedWriterPerformsAtMostOneWriteAndReconcilesEveryPhase(t *testing.T) {
	...
	request := phasedRequest(baseSHA, baseBlobSHA, treeSHA, baseContent, postContent)
	...
	assertOneWrite(t, &writes, func() error {
		...
		if callErr == nil && observation.Phase != remediation.WritePhaseEnsureCommit {
	...
	assertNoWrite(t, &writes, func() error {
		observation, callErr := client.EnsureBranch(context.Background(), request)
		if callErr == nil && (!observation.Reconciled || observation.Phase != remediation.WritePhaseEnsureCommit) {
	...
	assertOneWrite(t, &writes, func() error {
		...
		if callErr == nil && observation.Phase != remediation.WritePhaseEnsureDraftPR {
	...
	assertNoWrite(t, &writes, func() error {
		observation, callErr := client.EnsureCommit(context.Background(), request)
		if callErr == nil && (!observation.Reconciled || observation.Phase != remediation.WritePhaseEnsureDraftPR) {
	...
	assertOneWrite(t, &writes, func() error {
		...
		if callErr == nil && (observation.Phase != remediation.WritePhaseComplete || observation.PRNumber != 17) {
	...
	assertNoWrite(t, &writes, func() error {
		observation, callErr := client.EnsureDraftPR(context.Background(), request)
		if callErr == nil && (!observation.Reconciled || observation.Phase != remediation.WritePhaseComplete || observation.PRNumber != 17) {
...
func TestPhasedWriterRejectsUnboundPostImageBeforeIO(t *testing.T) {
	...
	request := phasedRequest(strings.Repeat("a", 40), strings.Repeat("b", 40), strings.Repeat("d", 40), []byte("before\n"), []byte("after\n"))
...
func phasedRequest(baseSHA, baseBlobSHA, treeSHA string, baseContent, postContent []byte) remediation.PhasedDeliveryRequest {
