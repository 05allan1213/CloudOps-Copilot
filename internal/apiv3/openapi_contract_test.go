...
func TestHandwrittenOpenAPIMatchesPhase2Routes(t *testing.T) {
	data, err := os.ReadFile("../../docs/api-v3-openapi.yaml")
	if err != nil {
		t.Fatal(err)
	}
	var document struct {
	...
	for _, route := range Phase2Routes() {
...
func TestOpenAPICommandAndSafetyContractsMatchRuntime(t *testing.T) {
	...
	if len(document.Security) == 0 || document.Security[0]["bearerAuth"] == nil {
		t.Fatal("OpenAPI must require bearerAuth globally")
	}

	commands := map[string]string{
		"/api/v3/incidents/{id}/investigations":    "#/components/schemas/VersionedCommand",
		"/api/v3/incidents/{id}/close":             "#/components/schemas/VersionedCommand",
		"/api/v3/remediation-plans/{id}/decisions": "#/components/schemas/DecisionCommand",
	}
	for path, bodySchema := range commands {
		...
		if !strings.Contains(strings.ToLower(operation.Description), "operator") {
			t.Errorf("command does not document operator role: %s", path)
		}
		parameterRefs := make(map[string]bool)
		for _, parameter := range operation.Parameters {
			parameterRefs[parameter.Ref] = true
		}
		for _, required := range []string{"#/components/parameters/IdempotencyKey", "#/components/parameters/CsrfToken", "#/components/parameters/Origin"} {
		...
		for _, status := range []string{"202", "400", "401", "403", "409", "413", "422", "501", "default"} {
	...
	for _, route := range Phase2Routes() {
		if route.Method != "GET" || route.Path == "/api/v3/session/csrf" {
			continue
		}
		path := strings.ReplaceAll(route.Path, ":id", "{id}")
