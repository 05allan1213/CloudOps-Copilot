...
func TestTypedWorkbenchProjectionEndpoints(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path:      "/api/v3/incidents/" + contractIncidentID + "/remediation-plans",
		...
		{
			path:      "/api/v3/incidents/" + contractIncidentID + "/delivery",
		...
		{
			path:      "/api/v3/incidents/" + contractIncidentID + "/verifications",
		...
		{
			path:      "/api/v3/incidents/" + contractIncidentID + "/resolution-report",
...
func TestTypedWorkbenchProjectionRejectsUnsafeOrUnboundedData(t *testing.T) {
	...
	plan = validRemediationPlanFixture()
	plan.Decision.Actor.Role = "admin"
	if err := validateRemediationPlanView(&plan); err == nil {
...
func validRemediationPlanFixture() RemediationPlanView {
	...
	item.Decision = &RemediationDecisionView{
		...
		Decision: "approved", Actor: RemediationDecisionActorView{Provider: "github", Login: "oncall", Role: "operator"},
