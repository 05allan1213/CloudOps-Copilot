...
SELECT p.id, p.public_id, p.cycle_no, p.v3_status, p.row_version,
...
func (p *MySQLQueryPort) listRemediationPlans(ctx context.Context, request QueryRequest) (QueryResponse, error) {
	...
	where := []string{
		"p.incident_id = ?", "p.cycle_no = ?", "p.domain_schema_version = 3",
	...
	if err != nil {
		return QueryResponse{}, fmt.Errorf("list V3 remediation plan projections: %w", err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return QueryResponse{}, fmt.Errorf("scan V3 remediation plan projection: %w", err)
		}
		all = append(all, item)
	}
	if err := rows.Err(); err != nil {
		return QueryResponse{}, fmt.Errorf("iterate V3 remediation plan projections: %w", err)
	}
	items := all
	next := ""
...
SELECT cr.public_id, cr.cycle_no, cr.v3_status, cr.row_version, p.public_id,
...
WHERE cr.incident_id = ? AND cr.cycle_no = ?
  AND cr.domain_schema_version = 3 AND cr.public_id IS NOT NULL
...
SELECT vr.id, vr.public_id, vr.cycle_no, vr.v3_status, vr.row_version,
...
LEFT JOIN change_requests cr
 ...
 AND cr.domain_schema_version = 3
...
func (p *MySQLQueryPort) listVerificationRuns(ctx context.Context, request QueryRequest) (QueryResponse, error) {
	...
	where := []string{
		"vr.incident_id = ?", "vr.cycle_no = ?", "vr.domain_schema_version = 3",
	...
	if err != nil {
		return QueryResponse{}, fmt.Errorf("list V3 verification projections: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err != nil {
			return QueryResponse{}, fmt.Errorf("scan V3 verification projection: %w", err)
		}
		all = append(all, item)
	}
	if err := rows.Err(); err != nil {
		return QueryResponse{}, fmt.Errorf("iterate V3 verification projections: %w", err)
	}
	next := ""
	...
	if err := validateVerificationRunViews(items); err != nil {
		return QueryResponse{}, fmt.Errorf("invalid V3 verification projection: %w", err)
	}
...
FROM (
    ...
    WHERE vc.incident_id = ? AND vc.cycle_no = ?
      AND vc.domain_schema_version = 3
...
ORDER BY verification_run_id, id`, args...)
	if err != nil {
		return fmt.Errorf("list V3 verification check projections: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err != nil {
			return fmt.Errorf("scan V3 verification check projection: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return fmt.Errorf("iterate V3 verification check projections: %w", err)
	}
...
ORDER BY verification_run_id, verification_check_id, sample_sequence`, args...)
	...
	if err := rows.Err(); err != nil {
		return fmt.Errorf("iterate V3 verification sample projections: %w", err)
	}
