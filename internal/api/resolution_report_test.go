...
func TestResolutionReportQueryUsesCurrentCycleProjection(t *testing.T) {
	...
	if strings.Contains(query, "domain_schema_version") {
		t.Fatal("ResolutionReport query still uses a generation discriminator")
	}
...
func TestResolutionReportHandlerReturnsCompleteSanitizedPostDeliveryProjection(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/api/v1/incidents/"+contractIncidentID+"/resolution-report", nil)
