...
// CORS is the browser boundary for the sole V1 contract.
func CORS(allowedOrigins []string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if !policy.originAllowed(c.Request) {
			...
			policy.writeProblem(c, http.StatusForbidden, "ORIGIN_FORBIDDEN", "Origin is not allowed for the API")
			c.Abort()
			return
		}
...
func writeCORSHeaders(c *gin.Context, origin string) {
	...
	c.Header(corsAllowHeaders, "Content-Type, X-Request-ID, Idempotency-Key, Last-Event-ID")
...
// Recovery keeps panic responses inside the stable problem contract.
...
// LimitRequestBody provides the contract 413 problem response and also bounds
...
// preserving the Problem Details representation on an actual rejection.
func RateLimit(store SlidingWindowLimiter, config RateLimitConfig) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		key := fmt.Sprintf("cloudops:rate_limit:api:%s:%s", c.ClientIP(), path)
