...
func (r *WorkspaceRepository) ProposeActionCard(ctx context.Context, request ActionProposalRequest) (ActionCard, error) {
	...
	}{"reversible", request.ActionType, request.Target, request.Parameters, request.Preconditions, request.Risk, request.ExpiresAt.UTC()})
	hash, publicID, now := workspaceSHA256(canonical), uuid.NewString(), r.now().UTC()
...
ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)`, publicID, runID, request.ActionType, request.Target,
		request.Parameters, request.Preconditions, request.Risk, hash, request.ExpiresAt.UTC(), now)
	if err != nil {
...
WHERE run.public_id=? AND run.run_kind='workspace' FOR UPDATE`, strings.TrimSpace(request.RunID)).Scan(
	...
	canonical, _ := json.Marshal(struct {
		...
		ExpiresAt             time.Time       `json:"expires_at"`
	}{"high_impact", revisionPublicID, request.ActionType, request.Target, request.Parameters, request.IntendedState,
		request.Preconditions, request.Risk, request.VerificationIntent, request.ExpiresAt.UTC()})
	hash, publicID, now := workspaceSHA256(canonical), uuid.NewString(), r.now().UTC()
...
ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)`, publicID, runID, revisionID, request.ActionType, request.Target,
		...
		hash, request.ExpiresAt.UTC(), now)
	if err != nil {
