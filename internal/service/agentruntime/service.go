...
func New(ctx context.Context, store agent.Store, model agent.Model, tools agent.ToolExecutor, cfg Config) (*Service, error) {
	if model == nil || tools == nil {
		return nil, fmt.Errorf("%w: model and tools are required", agent.ErrInvalidArgument)
	}
	...
	application, err := agentapplication.New(store, agentapplication.Config{
		Enabled:       cfg.Enabled,
		Model:         cfg.Model,
...
func (s *Service) processRun(ctx context.Context, run *agent.Run) error {
	ctx, span := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/agentruntime").Start(ctx, "agent.run")
...
func (s *Service) ExecuteNode(ctx context.Context, node agent.Node, state *agent.GraphState) (*agent.GraphState, error) {
	...
	ctx, span := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/agentruntime").Start(ctx, spanName(node))
	...
	if node == agent.NodePersistObservation {
		...
		checkpointCtx, checkpointSpan := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/agentruntime").Start(ctx, "agent.checkpoint.persist")
