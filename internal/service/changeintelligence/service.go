...
func (s *Service) Refresh(ctx context.Context, incidentID string) (Context, error) {
	...
	started := s.cfg.Now()
	ctx, span := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.context.load")
	defer span.End()
	incident, err := s.cfg.Incidents.GetByPublicID(ctx, incidentID)
	if err != nil {
		return Context{}, err
	...
	if s.cfg.Runtime != nil {
		runtimeCtx, runtimeSpan := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.runtime.resolve")
	...
	if s.cfg.Registry != nil && runtimeRepository != "" && runtime.ImageDigest != "" {
		registryCtx, registrySpan := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.registry.query")
	...
	argoCtx, argoSpan := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.argocd.query")
...
func (s *Service) buildCandidate(ctx context.Context, incident *domain.Incident, mapping ServiceMapping, app change.ArgoApplication, deployment change.ArgoHistory, runtime change.RuntimeContext, resolution change.ImageResolution, resolvedCommit *change.Commit) (change.Change, []string) {
	...
	if s.cfg.GitHub != nil && commitSHA.MatchString(deployment.Revision) {
		githubCtx, githubSpan := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.github.query")
...
func (s *Service) resolveImage(ctx context.Context, input change.ImageResolutionInput) change.ImageResolution {
	_, span := otel.Tracer("github.com/05allan1213/CloudOps-Copilot/internal/service/changeintelligence").Start(ctx, "change.image.resolve")
