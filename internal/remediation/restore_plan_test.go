...
func TestRemediationHintRejectsModelSuppliedAuthority(t *testing.T) {
	...
	hint, err := DecodeRemediationHint([]byte(valid))
	...
	for _, injected := range []string{
		...
		if _, err := DecodeRemediationHint([]byte(injected)); err == nil {
...
func TestCompileRestoreRequiredEnvBindsCompletePlan(t *testing.T) {
	...
	if plan.TargetBaseBranch != "main" || plan.IncidentVersion != 9 || plan.PlanContentSchemaVersion != PlanContentSchemaVersion || plan.TargetFieldRef != "spec.template.spec.containers[name=demo].env[name=REQUIRED_ENV]" || len(plan.ExpectedPostImageHash) != 64 || len(plan.ProposedPatchHash) != 64 || len(plan.EvidenceSetHash) != 64 || len(plan.VerificationPlanHash) != 64 {
	...
	changed := plan
	changed.BoundedDiff += "\n"
	changedHash, err := CanonicalPlanHash(changed)
...
func TestApprovalBindsEveryImmutableHash(t *testing.T) {
	plan, err := CompileRestoreRequiredEnv(validRestoreRequest())
	if err != nil {
		t.Fatal(err)
	}
	now := plan.CreatedAt.Add(10 * time.Minute)
	approval, err := NewApproval(plan, "local", "owner", "owner", "reviewed exact diff", "request-1", now, now.Add(10*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	if err := ValidateApprovalBinding(plan, approval, now.Add(time.Minute)); err != nil {
		t.Fatal(err)
	}
	if approval.DecisionSchemaVersion != DecisionSchemaVersion || approval.IncidentID != plan.IncidentID || approval.CycleNo != plan.CycleNo || approval.PlanVersion != plan.PlanVersion {
		t.Fatalf("decision ownership or schema binding missing: %+v", approval)
	}
	approval.ApprovedTreeHash = strings.Repeat("f", 40)
	if err := ValidateApprovalBinding(plan, approval, now.Add(time.Minute)); !errors.Is(err, ErrApprovalMismatch) {
		t.Fatalf("mutated approval error=%v", err)
	}
	approval.ApprovedTreeHash = plan.ExpectedTreeHash
	if err := ValidateApprovalBinding(plan, approval, approval.ExpiresAt); !errors.Is(err, ErrApprovalMismatch) {
		t.Fatalf("expired approval error=%v", err)
	}
}

func TestDecisionRejectsImportedIdentity(t *testing.T) {
	plan, err := CompileRestoreRequiredEnv(validRestoreRequest())
	if err != nil {
		t.Fatal(err)
	}
	now := plan.CreatedAt.Add(10 * time.Minute)
	if _, err := NewApproval(plan, "github", "operator-login", "operator", "reviewed exact diff", "request-imported", now, now.Add(10*time.Minute)); err == nil {
		t.Fatal("imported GitHub identity unexpectedly authorized a Decision")
	}
	approval, err := NewApproval(plan, "local", "owner", "owner", "reviewed exact diff", "request-owner", now, now.Add(10*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	approval.Role = "operator"
	if err := ValidateDecisionBinding(plan, approval, now.Add(time.Minute)); !errors.Is(err, ErrApprovalMismatch) {
		t.Fatalf("persisted non-Owner role error=%v", err)
	}
}
...
func validRestoreRequest() RestoreEnvCompileRequest {
	...
	policy := RestoreEnvPolicy{
		...
		MaxDiffBytes: MaxPlanDiffBytes, MaxPostImageBytes: MaxPostImageBytes, VerificationVersion: "golden-required-env/v1",
	}
