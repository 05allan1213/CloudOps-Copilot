...
func TestV3ProfilesAreFrozenDistinctAndLokiFree(t *testing.T) {
	input := validV3Input("post_delivery")
	golden, err := CompileV3VerificationPlan(input)
	if err != nil {
		t.Fatal(err)
	}
	if golden.ProfileID != GoldenRequiredEnvProfileID || len(golden.Checks) != 10 || golden.Deadline != V3RunDeadline {
		t.Fatalf("golden plan=%+v", golden)
	}
	if err := ValidateV3Plan(golden); err != nil {
		t.Fatal(err)
	}
	for _, check := range golden.Checks {
		if strings.Contains(strings.ToLower(check.SourceIdentity), "loki") || check.StabilityWindow != V3CommonStabilityWindow || check.MinSamples <= 0 {
			t.Fatalf("unsafe check=%+v", check)
		}
	}
	second, err := CompileV3VerificationPlan(input)
	...
	noChangeInput := validV3Input("no_change")
	noChange, err := CompileV3VerificationPlan(noChangeInput)
	if err != nil {
		t.Fatal(err)
	}
	...
	changed := GoldenRequiredEnvProfileV1()
	changed.Checks[6].Threshold = .02
	changedHash, err := V3ProfileHash(changed)
	if err != nil || changedHash == golden.ProfileHash {
		t.Fatal("profile hash did not change with threshold")
	}
}

func TestV3EvaluatorRequiresHealthAndMinimumSamples(t *testing.T) {
	...
	if got := EvaluateV3Observation(check, base, now); got.Status != SampleUnavailable || got.ReasonCode != "insufficient_samples" {
		t.Fatalf("under-sampled observation=%+v", got)
	}
	base.Denominator, base.SampleCount = 50, 50
	if got := EvaluateV3Observation(check, base, now); got.Status != SamplePassed {
		t.Fatalf("healthy metric=%+v", got)
	}
	base.SourceHealthy = false
	if got := EvaluateV3Observation(check, base, now); got.Status != SampleUnavailable {
	...
	if got := EvaluateV3Observation(logCheck, absence, now); got.Status != SamplePassed {
		t.Fatalf("healthy absence query=%+v", got)
	}
	absence.QueryValid = false
	if got := EvaluateV3Observation(logCheck, absence, now); got.Status != SampleUnavailable {
...
func TestV3CommonWindowUsesLatestSuccessStartAndInconclusiveUnavailable(t *testing.T) {
	...
	checks := []Check{
		{Required: true, Status: CheckRunning, ConsecutiveSuccessSince: &first, LastCheckedAt: ptrTimeV3(start.Add(69 * time.Second)), PollInterval: 10 * time.Second},
		{Required: true, Status: CheckRunning, ConsecutiveSuccessSince: &second, LastCheckedAt: ptrTimeV3(start.Add(69 * time.Second)), PollInterval: 10 * time.Second},
	}
	...
	checks[0].LastCheckedAt = ptrTimeV3(start.Add(5 * time.Minute))
	checks[1].LastCheckedAt = ptrTimeV3(start.Add(5 * time.Minute))
...
func TestV3ImmediateFailureAndResetSemantics(t *testing.T) {
	plan, err := CompileV3VerificationPlan(validV3Input("post_delivery"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := ApplyV3Sample(&identity, Sample{Status: SampleFailed, ReasonCode: "identity_mismatch"}, now); err != nil || identity.Status != CheckFailed {
	...
	if err := ApplyV3Sample(&metric, Sample{Status: SamplePassed}, now); err != nil || metric.Status != CheckRunning || metric.ConsecutiveSuccessSince == nil {
		t.Fatalf("first reset success=%+v err=%v", metric, err)
	}
	if err := ApplyV3Sample(&metric, Sample{Status: SamplePending, ReasonCode: "threshold_not_satisfied"}, now.Add(10*time.Second)); err != nil || metric.ConsecutiveSuccessSince != nil || metric.Status != CheckRunning {
		t.Fatalf("reset failure=%+v err=%v", metric, err)
	}
}

func validV3Input(trigger string) V3CompileInput {
...
func ptrTimeV3(value time.Time) *time.Time { return &value }
