...
test("Incident list keeps URL sort state and opens the read-only Inspector", async ({ page }) => {
  ...
  await page
    .getByTestId("incident-row-summary")
    .filter({ hasText: "Checkout API recovery" })
...
test("Incident detail renders the seven-stage single-owner lifecycle", async ({ page }) => {
  const browser = monitorBrowser(page, {
    allowedFailures: [/\/events: net::ERR_ABORTED$/, ...offlineIconFailures],
  });
  const mutations: string[] = [];
