...
test("Gate 10 converges Incident ownership and preserves non-incident DevOps commands", async ({ page }) => {
  ...
  await page.locator(".dense-data-table-row").filter({ hasText: "kubernetes.deployment.scale" }).click();
  await expect(page).toHaveURL(new RegExp(`selected=${incidentPlanID}`));
  await expect(page.getByTestId("devops-inspector")).toBeVisible();
  await expect(page.getByTestId("devops-inspector").getByText("Incident-owned", { exact: true })).toBeVisible();
  await expect(page.getByRole("link", { name: "Approval" })).toHaveAttribute("href", `/incidents/${incidentID}#approval`);
  await expect(page.getByRole("link", { name: "Delivery" })).toHaveAttribute("href", `/incidents/${incidentID}#delivery`);
  await expect(page.getByRole("link", { name: "Verification" })).toHaveAttribute("href", `/incidents/${incidentID}#verification`);
  ...
  await page.locator(".dense-data-table-row").filter({ hasText: "local.change_freeze.set" }).click();
  await page.getByRole("button", { name: "打开完整技术详情" }).click();
  await expect(page.getByTestId("non-incident-actions")).toBeVisible();
  await expect(page.getByRole("button", { name: "授权 exact hash" })).toBeVisible();

  expect(mutations).toEqual([]);
  browser.expectClean();
});

test("Gate 10 restores operation-only Query and renders Delivery Identity", async ({ page }) => {
  const browser = monitorBrowser(page, { allowedFailures: offlineIconFailures });
  const mutations: string[] = [];
  page.on("request", (request) => {
    if (!['GET', 'HEAD', 'OPTIONS'].includes(request.method())) mutations.push(`${request.method()} ${request.url()}`);
  ...
  await page.getByRole("tab", { name: "Delivery Identity" }).click();
  ...
  await expect(page.getByRole("heading", { name: "Delivery projection" })).toBeVisible();
