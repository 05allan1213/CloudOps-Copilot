...
const artifactPath = `objects/incident-decision-${decision || "undecided"}-${scenarioID || "scenario-required"}.json`;
...
interface IncidentDecisionArtifact {
  run_id: string;
  scenario_id: string;
  alert_id: string;
  incident_id?: string;
...
function currentArtifact(): IncidentDecisionArtifact {
  return readRunArtifact<IncidentDecisionArtifact>(artifactPath) ?? {
    run_id: runID,
...
function saveArtifact(patch: Partial<IncidentDecisionArtifact>) {
