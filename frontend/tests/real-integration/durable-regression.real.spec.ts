...
function requiredArtifact<T>(relativePath: string): T {
  ...
  if (!artifact) throw new Error(`${relativePath} is required for the durable regression`);
  return artifact;
}
...
test.describe.serial("durable real-browser regression", () => {
  test("all public routes reload in Live Mode with equivalent Scope and Settings", async ({ page }) => {
    ...
    recordRunArtifact("durable-regression/routes-settings.json", {
  ...
  test("run-scoped durable results survive cleanup without mutations", async ({ page }) => {
    ...
    expect(mutations, "durable regression attempted an API mutation").toEqual([]);
    recordRunArtifact("durable-regression/results.json", {
      observed_at: new Date().toISOString(),
      retained,
      api_evidence: reads,
      mutations,
