...
func (e *Executor) runK8sGetNodes(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	var query k8sreader.QueryOptions
	if err := json.Unmarshal(args, &query); err != nil {
		return ToolResult{}, NewInvalidArgsError("", "must be valid JSON")
	}
	nodes, err := e.k8sReader.ListNodes(ctx, query)
	if err != nil {
		return ToolResult{}, err
	}
	return ToolResult{Success: true, Data: nodes}, nil
}
