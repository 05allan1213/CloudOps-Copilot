...
func evidenceToRow(item *domain.EvidenceItem) evidenceRow {
	return evidenceRow{ID: item.ID, PublicID: item.PublicID, IncidentID: item.IncidentID, AgentRunID: item.AgentRunID, Type: item.Type, Source: item.Source, ResourceRef: item.ResourceRef, TimeRangeJSON: item.TimeRange, QueryText: item.Query, Summary: item.Summary, FactsJSON: item.Facts, RawRef: item.RawRef, Truncated: item.Truncated, CollectedAt: item.CollectedAt, CreatedAt: item.CreatedAt}
}
