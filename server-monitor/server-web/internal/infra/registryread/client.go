...
func (c *Client) ReadMetadata(ctx context.Context, repository, manifestDigest string) (change.RegistryMetadata, error) {
	ctx, span := otel.Tracer("server-web/internal/infra/registryread").Start(ctx, "registry.metadata.read")
	defer span.End()
	...
	if value, ok := c.cached(key); ok {
		c.observeCache("hit")
		_, cacheSpan := otel.Tracer("server-web/internal/infra/registryread").Start(ctx, "registry.cache")
...
func (c *Client) readUncached(ctx context.Context, repository, manifestDigest string) (change.RegistryMetadata, error) {
	...
	manifestCtx, manifestSpan := otel.Tracer("server-web/internal/infra/registryread").Start(ctx, "registry.manifest.read")
	...
	configCtx, configSpan := otel.Tracer("server-web/internal/infra/registryread").Start(ctx, "registry.config.read")
...
func (c *Client) exchangeChallenge(ctx context.Context, repository, raw string) (tokenResult string, retErr error) {
	ctx, span := otel.Tracer("server-web/internal/infra/registryread").Start(ctx, "registry.auth")
	defer func() {
