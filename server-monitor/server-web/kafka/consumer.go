...
func (h *consumerGroupHandler) processMessage(ctx context.Context, marker messageMarker, msg *sarama.ConsumerMessage) {
	...
	ctx, span := otel.Tracer("server-web/kafka").Start(ctx, "server-web.diagnosis.consume")
	defer span.End()
	span.SetAttributes(
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			logger.FromContext(ctx).Error("process diagnosis alert event panic recovered, skipping offset commit",
	...
	if err := json.Unmarshal(msg.Value, &event); err != nil {
		logger.FromContext(ctx).Warn("unmarshal diagnosis alert event failed",
			zap.String("topic", msg.Topic),
			zap.Int32("partition", msg.Partition),
			zap.Int64("offset", msg.Offset),
			zap.Error(err),
	...
	if err := h.processor.Process(ctx, event); err != nil {
		...
		logger.FromContext(ctx).Error("process diagnosis alert event failed, skipping offset commit",
			zap.String("topic", msg.Topic),
			zap.Int32("partition", msg.Partition),
			zap.Int64("offset", msg.Offset),
			zap.String("fingerprint", event.Fingerprint),
