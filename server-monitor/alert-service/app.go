...
func initApp(ctx context.Context) (*app, error) {
	...
	infra := initInfrastructure(ctx, cfg)
	svc, err := initServices(cfg, infra)
	if err != nil {
		return nil, err
	}
...
// initInfrastructure creates process-level clients that must be released on shutdown.
func initInfrastructure(ctx context.Context, cfg config.Config) infrastructure {
...
// initServices assembles business services and HTTP/Kafka entrypoints from infrastructure.
func initServices(cfg config.Config, infra infrastructure) (services, error) {
	serviceMetrics := servicemetrics.New()
	store := alert.NewStore(infra.redisClient, alert.DefaultDedupTTL, serviceMetrics)
	...
	if err != nil {
		return services{}, fmt.Errorf("kafka consumer init failed: %w", err)
	}
