...
run_golden() {
  ...
  final_incidents="$(wait_for_json 'api_get "/api/v3/incidents?service=cloudops-demo-workload&limit=100"' '[.items[]|select(.created_at>="'"${bad_merge_at}"'")]|length>0')" || fail incident "no current-cycle incident appeared after the bad merge"
  INCIDENT_ID="$(jq -r --arg at "${bad_merge_at}" '[.items[]|select(.created_at >= $at)]|sort_by(.created_at)|last.id' <<<"${final_incidents}")"; is_uuid "${INCIDENT_ID}" || fail incident "incident public ID is invalid"; incident_at="$(jq -r --arg id "${INCIDENT_ID}" '.items[]|select(.id==$id)|.created_at' <<<"${final_incidents}")"; pass incident "Incident ${INCIDENT_ID} detected"
  plans="$(wait_for_json 'api_get "/api/v3/incidents/'"${INCIDENT_ID}"'/remediation-plans?limit=100"' '[.items[]|select(.operation_type=="restore_required_env" and .decision.decision=="approved")]|length==1')" || fail plan_approval "no unique approved restore_required_env plan appeared"
  plan="$(jq -c '[.items[]|select(.operation_type=="restore_required_env" and .decision.decision=="approved")][0]' <<<"${plans}")"; PLAN_ID="$(jq -r .id <<<"${plan}")"; AGENT_RUN_ID="$(jq -r .created_by_agent_run_id <<<"${plan}")"; APPROVAL_ID="$(jq -r .decision.id <<<"${plan}")"; APPROVED_TREE_HASH="$(jq -r .decision.approved_tree_hash <<<"${plan}")"; APPROVED_POST_IMAGE_HASH="$(jq -r .decision.approved_post_image_hash <<<"${plan}")"; EVIDENCE_IDS="$(jq -r '[.evidence_bindings[].id]|join(",")' <<<"${plan}")"; agent_at="$(jq -r .created_at <<<"${plan}")"; approval_at="$(jq -r .decision.created_at <<<"${plan}")"; pass agent "AgentRun ${AGENT_RUN_ID} produced one bounded plan"; pass plan_approval "Plan ${PLAN_ID}, approval ${APPROVAL_ID}, hashes bound"
  delivery="$(wait_for_json 'api_get "/api/v3/incidents/'"${INCIDENT_ID}"'/delivery"' '.resource.status=="delivered" and .resource.merged_commit_sha!=""')" || fail remediation_pr "delivery did not reach delivered"
