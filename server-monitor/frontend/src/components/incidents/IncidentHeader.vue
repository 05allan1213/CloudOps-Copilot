...
defineProps<{ incident: IncidentDTO; realtimeState: string }>();
</script>
...
<template>
  <header class="incident-header">
    <div>
      ...
      <p>{{ incident.triggering_summary || "Unknown triggering signal" }}</p>
    </div>
    <div
      class="header-badges"
    ...
    <dl class="header-facts">
      <div><dt>Service / environment</dt><dd>{{ incident.service || "Unknown" }} / {{ incident.environment || "Unknown" }}</dd></div>
      <div><dt>Namespace / workload</dt><dd>{{ incident.namespace || "Unknown" }} / {{ incident.workload_name || "Unknown" }}</dd></div>
      ...
      <div><dt>Resolved</dt><dd>{{ formatIncidentTime(incident.resolved_at) }}</dd></div>
    </dl>
  </header>
</template>
