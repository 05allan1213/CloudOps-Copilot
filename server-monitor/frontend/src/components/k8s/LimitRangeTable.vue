...
<template>
  <el-table :data="limitranges" stripe highlight-current-row style="width: 100%">
    ...
    <el-table-column label="限制" min-width="300">
      <template #default="{ row }">
        <div v-for="(item, idx) in row.limits" :key="idx" class="limit-row">
          <el-tag size="small" type="info">{{ item.type }}</el-tag>
          <span v-if="item.min" class="limit-field">Min: {{ item.min }}</span>
          <span v-if="item.max" class="limit-field">Max: {{ item.max }}</span>
          <span v-if="item.default" class="limit-field">Default: {{ item.default }}</span>
        </div>
      </template>
    </el-table-column>
