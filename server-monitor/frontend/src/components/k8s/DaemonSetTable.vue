...
<template>
  <el-table :data="items" v-loading="loading" stripe style="width: 100%">
    ...
    <el-table-column label="Desired" width="90" align="center">
      <template #default="{ row }">{{ row.desired }}</template>
    </el-table-column>
    ...
    <el-table-column label="Updated" width="90" align="center">
      <template #default="{ row }">{{ row.updated }}</template>
    </el-table-column>
    <el-table-column prop="node_selector" label="Node Selector" min-width="160" show-overflow-tooltip>
      <template #default="{ row }">
        {{ row.node_selector || "-" }}
      </template>
    </el-table-column>
